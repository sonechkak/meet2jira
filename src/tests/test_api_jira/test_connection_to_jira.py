...
def test_jira_client_initialization(jira_client):
    ...
    assert jira_client.server_url == os.getenv(
        "JIRA_API_URL"
    ), "Jira server URL does not match the expected value."
...
def test_jira_client_authentication(jira_client):
    ...
    """
    assert (
        jira_client._session.auth is not None
    ), "Jira client should have authentication credentials."
    assert jira_client._session.auth[0] == os.getenv(
        "JIRA_API_USER"
    ), "Jira username does not match the expected value."
    assert jira_client._session.auth[1] == os.getenv(
        "JIRA_API_TOKEN"
    ), "Jira API token does not match the expected value."
...
def test_create_minimal_issue(jira_client):
    ...
    jira_service = JiraService(
        os.getenv("JIRA_API_URL"),
        os.getenv("JIRA_API_USER"),
        os.getenv("JIRA_API_TOKEN"),
    )
    try:
        task_key = jira_service.create_minimal_task("Тестовая задача", "MEET2JIRA")
